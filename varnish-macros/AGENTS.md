...
- Any codegen change touches insta snapshots under `varnish/snapshots*` — expect large diffs. `varnish/snapshots*` has more entries than this, but only `snapshots8.0.0`, `snapshots9.0.0`, `snapshotstrunk` are real directories holding actual snapshot data — every other entry is a symlink to one of those three (see [varnish/tests/AGENTS.md](../varnish/tests/AGENTS.md)), so you don't need to bless each one separately per patch release.
