...
1. `detect_varnish()`: if `VARNISH_INCLUDE_PATHS` env set (colon-separated), use directly, **assume latest supported version** (can't auto-detect version this way — silently wrong if you're actually on an older lib). Else `pkg_config::Config::new().probe("varnishapi")` — gets real version from `.pc` file.
...
3. Emits version cfgs, e.g. `cfg(varnishsys_90_sslflags)` for >=9.0.0 or `trunk`. Warns if <8.0.0 (unsupported). New version-gated cfgs follow "name after last version where feature absent" convention.
...
5. Writes `OUT_DIR/bindings.rs`, diffs vs checked-in `bindings.for-docs`. Emits `cargo::warning` if they differ, telling dev to `cp $OUT_DIR/bindings.rs varnish-sys/bindings.for-docs` and bump `BINDINGS_FILE_VER` if version changed.
