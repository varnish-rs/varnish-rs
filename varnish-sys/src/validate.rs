...
#[cfg(feature = "full")]
pub use full::{
    validate_director, validate_vdir, validate_vfp_ctx, validate_vfp_entry, validate_ws,
};
...
// Everything below validates pointers into cache.h-only structs (`ws`, `req`, `sess`,
// `director`, `vcldir`, `vfp_ctx`, `vfp_entry`), unavailable under the vrt-only surface.
// Grouped into one module so the `full` gate isn't repeated on every item.
#[cfg(feature = "full")]
mod full {
    use crate::ffi::{
        self, director, req, sess, vcldir, vfp_ctx, vfp_entry, ws, DIRECTOR_MAGIC, REQ_MAGIC,
        SESS_MAGIC, VCLDIR_MAGIC, VCL_BACKEND, VFP_CTX_MAGIC, VFP_ENTRY_MAGIC, WS_MAGIC,
    };
