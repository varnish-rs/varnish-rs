...
#[cfg(feature = "full")]
pub use full::{log, BodyState, TestCtx};
...
pub struct Ctx<'a> {
    ...
    #[cfg(feature = "full")]
    pub http_req: Option<crate::vcl::HttpHeaders<'a>>,
    #[cfg(feature = "full")]
    pub http_req_top: Option<crate::vcl::HttpHeaders<'a>>,
    #[cfg(feature = "full")]
    pub http_resp: Option<crate::vcl::HttpHeaders<'a>>,
    #[cfg(feature = "full")]
    pub http_bereq: Option<crate::vcl::HttpHeaders<'a>>,
    #[cfg(feature = "full")]
    pub http_beresp: Option<crate::vcl::HttpHeaders<'a>>,
    pub ws: Workspace<'a>,
...
impl<'a> Ctx<'a> {
    ...
    pub fn from_ref(raw: &'a mut vrt_ctx) -> Self {
        ...
        Self {
            #[cfg(feature = "full")]
            http_req: crate::vcl::HttpHeaders::from_ptr(raw.http_req),
            #[cfg(feature = "full")]
            http_req_top: crate::vcl::HttpHeaders::from_ptr(raw.http_req_top),
            #[cfg(feature = "full")]
            http_resp: crate::vcl::HttpHeaders::from_ptr(raw.http_resp),
            #[cfg(feature = "full")]
            http_bereq: crate::vcl::HttpHeaders::from_ptr(raw.http_bereq),
            #[cfg(feature = "full")]
            http_beresp: crate::vcl::HttpHeaders::from_ptr(raw.http_beresp),
...
#[derive(Debug)]
pub struct Req<'a> {
    // Under vrt, `ffi::req` is opaque, so nothing reads `raw` (all field-accessing methods
    // live in the `full`-only impl below) — it's still stored so `ctx.req()`/`req_mut()` work.
...
// Everything below needs cache.h-only APIs (VSL logging, VSA, request-body iteration, VSB,
// concrete `req`/`ws` field access) unavailable under the vrt-only surface. Grouped into one
// module so the `full` gate isn't repeated on every item.
#[cfg(feature = "full")]
mod full {
    ...
    use crate::ffi;
    use crate::ffi::{vrt_ctx, VRT_CTX_MAGIC};
    use crate::vcl::subroutine::Id;
    use crate::vcl::{sc_to_ptr, Acl, Buffer, LogTag, StreamClose, TestWS, VclError, VclResult};
    ...
    impl<'a> Ctx<'a> {
        ...
        pub fn response_buffer(&mut self) -> VclResult<Buffer<'_>> {
            match self.subroutine() {
                ...
                _ => Err(
                    "ctx.response_buffer() is only available in vcl_synth or vcl_backend_error"
                        .into(),
                ),
            }
        }
    ...
    /// used elsewhere.
    ///
    /// Only available under `full`: it relies on [`TestWS`], which needs a concrete `struct ws`
    /// layout unavailable under the vrt-only surface.
    #[derive(Debug)]
    pub struct TestCtx {
        vrt_ctx: vrt_ctx,
        test_ws: TestWS,
