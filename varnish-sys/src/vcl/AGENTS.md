...
- `ctx.rs` — `Ctx`, `Req`, `TestCtx`, `PerVclState<T>`, `log()`. Main request-context wrapper around `vrt_ctx`. Has unit tests. Also holds `Ctx::cached_req_body()` (client `req` body, cached-only) and the backend-side pair `Ctx::req_body_status()`/`Ctx::req_body_read()` + `BodyStatus` enum (bereq's body, cached or streamed from the client — mirrors `body_status_t`/`V1F_SendReq` in Varnish itself). The backend pair calls into `backend/`'s `sc_to_ptr`/`StreamClose` for `doclose` bookkeeping, despite living in this file, not `backend/backend_main.rs` — kept next to `cached_req_body` for symmetry (client-body vs bereq-body reading), not split by module.
