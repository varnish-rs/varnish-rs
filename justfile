...
bless install_dir='' *args='-p varnish-macros -p varnish':  (cargo-install 'cargo-insta')
    #!/usr/bin/env bash
    set -euo pipefail
...
bless-all:
    ...
    {{just_executable()}} docker-run trunk "just bless /tmp/varnish-trunk"
...
get-varnish-version $required_version='':
    #!/usr/bin/env bash
    set -euo pipefail
    VARNISH_VER=$(varnishd -V 2>&1 | sed -n 's/.*varnish-\([0-9][^ )]*\).*/\1/p' | head -1)
    if [ -z "$VARNISH_VER" ]; then
        ...
        echo "Found varnishd v$VARNISH_VER"
    fi
...
# For `trunk`, clones https://github.com/varnish/varnish.git, builds, and installs to `install_dir`.
# This could be damaging to your system - use with caution. Pass non-empty `debug` argument to skip the installation.
[private]
install-varnish version=default_varnish_ver install_dir='/tmp/varnish-trunk' debug='':
    #!/usr/bin/env bash
    set -euo pipefail
